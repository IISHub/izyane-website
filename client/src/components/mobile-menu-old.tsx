  ...
  return (
    <>
      ...
      <AnimatePresence>
        {isOpen && (
          <motion.div
            className="mobile-menu fixed top-0 right-0 h-full w-80 max-w-[85vw] bg-white/95 dark:bg-slate-900/95 backdrop-blur-xl shadow-2xl z-50 md:hidden"
            initial={{ x: "100%" }}
            animate={{ x: dragProgress > 0 ? `${dragProgress * 100}%` : 0 }}
            exit={{ x: "100%" }}
            transition={{ 
              type: "spring",
              stiffness: 300,
          ...
          >
            ...
            {/* Drag Indicator */}
            <div className="absolute left-0 top-1/2 -translate-y-1/2 w-1 h-12 bg-slate-300 dark:bg-slate-600 rounded-r-full opacity-50" />
              ...
              {navLinks.map((link, index) => {
                ...
                return (
                  <motion.button
                    key={link.href}
                    onClick={() => handleNavClick(link.href, link.route)}
                    className="w-full flex items-center gap-4 px-6 py-4 text-left hover:bg-slate-100 dark:hover:bg-slate-800 transition-colors group"
                    initial={{ opacity: 0, x: 50 }}
                    animate={{ opacity: 1, x: 0 }}
                    transition={{ delay: index * 0.1 }}
                    whileTap={{ scale: 0.98 }}
                  >
                    <div className="p-2 rounded-lg bg-primary-custom/10 group-hover:bg-primary-custom/20 transition-colors">
                      <IconComponent className="w-5 h-5 text-primary-custom" />
                    </div>
                    <span className="text-lg font-medium text-slate-700 dark:text-slate-200 group-hover:text-primary-custom transition-colors">
                      {link.label}
                    </span>
                  </motion.button>
                );
            ...
            <div className="absolute bottom-0 left-0 right-0 p-6 border-t border-slate-200 dark:border-slate-700">
              ...
              >
                <p>© 2025 iZyane</p>
                <p className="mt-1 text-xs">Swipe right to close</p>
              </motion.div>
            </div>
